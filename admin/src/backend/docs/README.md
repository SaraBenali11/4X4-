Backend layout (MVC + layered)

Folders:

- `controllers/` - Flask Blueprints and request handlers (controllers)
- `services/` - Business logic and orchestration
- `repositories/` - Database access layer (ORM queries)
- `integration_tests/` - Integration test runners
- `tests/` - unit/integration tests
- `scripts/` - utility scripts

Database package has been moved to `src/database` and is imported as `database`.

Running:

- Start app: `python src/backend/app.py` (the app adds `src` to `sys.path` at runtime)
