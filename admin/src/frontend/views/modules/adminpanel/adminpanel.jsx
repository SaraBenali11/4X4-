...
import { stats } from "../../../../database/models/listsfortesting.js";
...
export default function AdminPanel() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const transformedOrders = data.map((order) => {
        ...
        const sizes =
          order.order_items?.map((item) => item.size).join(", ") || "N/A";
        ...
        if (normalizedStatus.toLowerCase() === "en attente") {
          ...
          normalizedStatus.toLowerCase() === "livrée" ||
          normalizedStatus.toLowerCase() === "livree"
        ) {
  ...
  const handleOrderStatusChange = async (orderId, newStatus) => {
    try {
      ...
      // Remove accents for database compatibility
      const statusMap = {
      ...
      dbStatus = statusMap[dbStatus] || dbStatus;

      console.log(`Updating order ${orderId} to status: ${dbStatus}`);
      await orderService.updateOrderStatus(orderId, dbStatus);
      console.log(`Order ${orderId} updated successfully`);

      // Refetch orders to ensure all sections are updated correctly
      await fetchOrders();
    } catch (error) {
  ...
  const filteredOrders =
    statusFilter === "all"
      ...
      : orders.filter((order) => {
          ...
          // Handle "livrée" vs "livree" comparison
          if (filterLower === "livrée") {
            return statusLower === "livrée" || statusLower === "livree";
          }
