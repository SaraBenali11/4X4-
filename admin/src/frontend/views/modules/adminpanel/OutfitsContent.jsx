...
export default function OutfitsContent() {
  ...
  const confirmDelete = async () => {
    const id = deleteConfirm.outfitId;
    setDeleteConfirm({ isOpen: false, outfitId: null, outfitTitle: "" });
  ...
  if (loading)
    return <div className="loading-state">Chargement des tenues...</div>;
  ...
  return (
    <div className="outfits-content">
            ...
            {outfits.length === 0 ? (
              <tr>
                <td colSpan="6" className="empty-row">
                  Aucune tenue trouvée. Cliquez sur "Ajouter une Tenue" pour en
                  créer une.
                </td>
              </tr>
            ) : (
              outfits.map((outfit) => (
                <tr key={outfit.id}>
                  ...
                  <td className="outfit-desc-cell">
                    {outfit.description || "-"}
                  </td>
                  <td className="outfit-products-cell">
                    {outfit.products && outfit.products.length > 0 ? (
                      <span className="products-count">
                        {outfit.products.length} produit
                        {outfit.products.length > 1 ? "s" : ""}
                      </span>
                    ) : (
      ...
      <ConfirmModal
        ...
        message={`Êtes-vous sûr de vouloir supprimer "${deleteConfirm.outfitTitle}"? Cette action est irréversible.`}
