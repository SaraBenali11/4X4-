...
export const orderService = {
  ...
  async updateOrderStatus(orderId, status) {
    try {
      console.log(
        `Attempting to update order ${orderId} with status: ${status}`
      );
    ...
    try {
      ...
      const itemsList =
        order.order_items
          ?.map(
            (item) =>
              `${item.product?.name || "Produit"} - Taille: ${item.size} × ${
                item.quantity
              }`
          )
          .join("\n") || "Produits commandés";
      ...
      const total =
        order.order_items?.reduce(
          (sum, item) =>
            sum + (item.price_at_order || 0) * (item.quantity || 0),
          0
        ) || 0;
...
Sutraty Store`;
      ...
      console.log(
        "✅ Rejection email sent successfully. Status:",
        emailResult.status
      );
